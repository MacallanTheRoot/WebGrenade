...
**✨ 100% Strict Cross-Browser Compatible (Chrome MV3 & Firefox MV3) ✨**

WebGrenade is a fast, offline-first browser utility suite for developers. It packs 9+ tools into a single popup without any of the tracking or bloat.

Everything stays locally in `chrome.storage`. No tracking, no cloud dependencies.
- **Custom Draggable & Resizable UI**: Drag the popup dashboard anywhere on your screen or resize its height up to 600px for max visibility!
...
## ⚡ Core Arsenal: 9 Professional Modules
...
| **🎤 Genius Lyrics** | `Background Fetch` | Instant song lyrics matching with Background API routing to bypass CORS and find song lyrics fast |
...
### 🎤 Module 2: Genius Lyrics

**Purpose**: Instantly find and read lyrics for your favorite songs.

**Features**:
- **Genius API Integration**:
  - fast search queries routed through `background.js` to bypass CORS
  - Extracts lyrics, album art, and song metadata
  - Clean and scrollable UI

**How to Use**:
1. Open WebGrenade → **Lyrics** tab
2. Search for a song or artist
3. View lyrics instantly in the extension

---

### 🔗 Module 4: Link Station
...
9. **Fake Input Filler**:
   - Integrated into Context Menu (`contexts: ["editable"]`) and Utilities card
   - Instantly populates emails, passwords, names, and numbers into web forms
   - Massive time-saver for testing inputs and QA
...
├── popup.js            # Popup logic: all 9 modules
...
**✨ %100 Katı Tarayıcılar Arası Uyumlu (Chrome MV3 & Firefox MV3) ✨**

WebGrenade geliştiriciler için hızlı ve izleme yapmayan bir araç setidir. — iş akışınızı karmaşıklaştırmadan **8+ temel araca anında erişim** gerektiren web profesyonelleri, geliştiriciler ve ileri düzey kullanıcılar için eksiksiz bir taktik araç setidir.

Veriler yerel `chrome.storage` üzerinde kalır. Telemetri veya bulut tabanlı bir sistem kullanmaz.
- **Özel Sürüklenebilir ve Yeniden Boyutlandırılabilir UI**: Popup panosunu ekranınızda istediğiniz yere sürükleyin veya maksimum görünürlük için yüksekliğini 600px'e kadar yeniden boyutlandırın!
...
## ⚡ Temel Cephanelik: 9 Profesyonel Modül
...
| **🎤 Genius Lyrics** | `Background Fetch` | Şarkı sözü eşleştirme, CORS aşmak için arka plan API yönlendirmesi ile anında şarkı sözü bulma |
...
### 🎤 Modül 2: Genius Lyrics

**Amaç**: En sevdiğiniz şarkıların sözlerini anında bulun ve okuyun.

**Özellikler**:
- **Genius API Entegrasyonu**:
  - CORS'u aşmak için `background.js` üzerinden yönlendirilen hızlı arama sorguları
  - Şarkı sözleri, albüm resmi ve şarkı meta verilerini çıkarır
  - Temiz ve kaydırılabilir kullanıcı paneli

**Nasıl Kullanılır**:
1. WebGrenade'i açın → **Lyrics** sekmesi
2. Bir şarkı veya sanatçı arayın
3. Eklentide anında şarkı sözlerini görüntüleyin

---

### 🔗 Modül 4: Link İstasyonu
...
8. **Fake Input Filler (Sahte Veri Doldurucu)**:
   - Bağlam Menüsüne (`contexts: ["editable"]`) ve Yardımcı Araçlar kartına entegre edilmiştir
   - Web formlarına e-posta, şifre, isim ve sayıları anında doldurur
   - Form testi ve QA için büyük zaman tasarrufu
...
## �🚫 Gelişmiş Popup Engelleyici (v4.0.0)
...
├── popup.js            # Popup mantığı: tüm 9 modül
